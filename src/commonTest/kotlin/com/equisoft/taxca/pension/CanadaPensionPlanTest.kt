...
class CanadaPensionPlanTest {
    ...
    fun getRequestDateFactor() {
        val bonus = Cpp.monthlyDelay.bonus
        val penalty = Cpp.monthlyDelay.penalty
        val cases = listOf(
            Case(
            ...
            Case(
                "should return 1 + 60 times the MONTHLY delay bonus when waiting has max age on request",
                birthDate = LocalDate(1980, 1, 1),
                requestDate = LocalDate(2055, 1, 1), // on his 70th birthday
                expected = 1 + (60 * bonus),
            ),
            Case(
                "should return max delay bonus (1 + 60 times the MONTHLY delay bonus) when request is after max age",
                birthDate = LocalDate(1980, 1, 1),
            ...
            Case(
                ...
                requestDate = LocalDate(2020, 1, 1), // on his 72nd birthday, past the 70 max request age
                expected = 1 + (60 * bonus),
            ),
            Case(
        ...
        for ((label, birthDate, requestDate, expected, customReferenceDate) in cases) {
            assertEquals(expected, Cpp.getRequestDateFactor(birthDate, requestDate, customReferenceDate), label)
        }
    }
