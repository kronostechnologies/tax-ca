...
describe('getRequestDateFactor', () => {
    ...
    it('should count the bonus from the reference age, not from the last birthday', () => {
        ...
        expect(ratio).toBe(1 + (14 * QPP.MONTHLY_DELAY.BONUS));
    });
    ...
    it('should count the bonus from the reference age for a request several years past 65', () => {
        ...
        expect(ratio).toBe(1 + (32 * QPP.MONTHLY_DELAY.BONUS));
    });
    ...
    it('should clamp to the max bonus when the request date is past the maximum request age', () => {
        ...
        const requestDate = new Date('2020-01-01'); // on his 72nd birthday, exactly the QPP max request age
    ...
    describe('ABF-13000 regression: bonus for clients already past the reference age', () => {
        it('should bonify a request on the 66th birthday (DOB 1960-01-01)', () => {
            ...
            expect(ratio).toBe(1 + (12 * QPP.MONTHLY_DELAY.BONUS)); // 1.084
        });
        ...
        it('should bonify a request on the 68th birthday (DOB 1958-01-01)', () => {
            ...
            expect(ratio).toBe(1 + (36 * QPP.MONTHLY_DELAY.BONUS)); // 1.252
        });
        ...
        it('should clamp to the max bonus at age 72 when the request date is past the maximum request age', () => {
            const birthDate = new Date('1980-01-01');
            const requestDate = new Date('2058-01-01'); // on his 78th birthday, past the 72 max request age

            const ratio = QPP.getRequestDateFactor(birthDate, requestDate);

            expect(ratio).toBe(1 + (84 * QPP.MONTHLY_DELAY.BONUS)); // 1.588
        });
